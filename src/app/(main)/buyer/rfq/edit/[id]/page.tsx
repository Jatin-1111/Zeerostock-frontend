...
import { Send, AlertCircle, Check, ChevronDown, ArrowLeft } from "lucide-react";
...
export default function EditRFQPage() {
  ...
  // Load RFQ data, categories and industries on mount
  useEffect(() => {
    const loadData = async () => {
      try {
        const [rfqRes, categoriesRes, industriesRes] = await Promise.all([
        ...
        if (rfqRes.success && rfqRes.data) {
          ...
          setFormData({
            ...
            industryId: rfq.industryId || "",
            quantity: rfq.quantity || 0,
            unit: (rfq.unit || "pieces") as RFQUnit,
            budgetMin: rfq.budgetMin,
            budgetMax: rfq.budgetMax,
            requiredByDate: rfq.requiredByDate
              ? new Date(rfq.requiredByDate).toISOString().split("T")[0]
              : "",
            detailedRequirements: rfq.detailedRequirements || "",
            preferredLocation: rfq.preferredLocation || "",
            durationDays: rfq.durationDays || 7,
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Redirect to my RFQs page after 2 seconds
        setTimeout(() => {
          router.push("/buyer/rfq/my-rfqs");
        }, 2000);
      } else {
        // Check if error is due to accepted quotes
        if (response.error?.code === "RFQ_HAS_ACCEPTED_QUOTES") {
          ...
          setError(response.message || "Failed to update RFQ");
          toast.error(response.message || "Failed to update RFQ");
        }
      }
    } catch (err: unknown) {
      const errorMessage =
        (err as Error).message || "An error occurred while updating the RFQ";
  ...
  return (
    <div className="flex flex-col justify-center items-center px-4 sm:px-6 md:px-8 lg:px-11 py-8 sm:py-10">
      <div className="w-full max-w-[750px] mx-auto">
        ...
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-[13px] text-gray-600 hover:text-gray-900 mb-4 transition-colors"
        >
        ...
        {success && (
          <div className="mb-3 sm:mb-4 p-2 bg-green-50 border border-green-200 flex items-center gap-1.5 rounded-[6px]">
            ...
            <p className="text-[11px] sm:text-xs text-green-800">
              RFQ updated successfully! Redirecting...
            </p>
          </div>
        )}
