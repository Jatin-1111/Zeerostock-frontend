...
export default function UniqueFeaturesSectionV2() {
  ...
  return (
    <section className="w-full py-4 sm:py-5 md:py-[23px] px-4 sm:px-6 md:px-8 lg:px-[40px]">
      {/* Title - 75% scaled (50px → 38px) */}
      <h2 className="text-center text-xl sm:text-2xl md:text-2xl font-bold mb-2 text-[#0d1b2a] leading-normal">
        <span>Unique Feature That </span>
        <span className="text-[#2ec096]">Drive Results</span>
      </h2>

      {/* Subtitle - 75% scaled (24px → 18px) */}
      <p className="text-center text-xs sm:text-sm md:text-sm font-semibold text-[#6b7280] mb-4 sm:mb-6 md:mb-[36px] leading-normal max-w-full sm:max-w-md md:max-w-[527px] mx-auto px-4 sm:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 sm:gap-x-6 md:gap-x-[39px] gap-y-3 sm:gap-y-4 md:gap-y-[20px] max-w-full sm:max-w-lg md:max-w-[639px] mx-auto">
        {features.map((feature, index) => (
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <h3 className="text-sm sm:text-base md:text-base font-semibold text-[#0d1b2a] leading-tight mb-0.5">
                {feature.title}
              </h3>
              {/* Description - 75% scaled (23px → 17px) */}
              <p className="text-xs sm:text-xs md:text-xs font-semibold text-[#787878] leading-normal">
                {feature.description}
              </p>
            </div>
          </div>
